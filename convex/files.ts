...
export const getFolderContent = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Sort: folders first, then files, alpahabetically within each group
...
export const createFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if file with same name in this parent folder
    const files = await ctx.db
      .query("files")
      .withIndex("by_project_parent", (q) =>
        q.eq("projectId", args.projectId).eq("parentId", args.parentId)
...
export const createFolder = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if folder with same name in this parent folder
    const files = await ctx.db
      .query("files")
      .withIndex("by_project_parent", (q) =>
        q.eq("projectId", args.projectId).eq("parentId", args.parentId)
